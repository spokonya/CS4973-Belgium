...
![Escape Room](https://www.debesteescaperooms.be/sites/default/files/2021-05/608b24546825c_thumb900.jpg)

### Deliverable 4 Update

#### Backend
On the backend I cleaned up the features and ran a bunch of checks before trusting the model. The main thing I caught was a multicollinearity issue with our interaction term. The raw storage_at_start × storage_volatility feature was 0.95-correlated with storage_volatility and pushed the VIF up to 43.6, which made the coefficients basically useless. I fixed it by mean-centering both features before multiplying, which brought every VIF back down to around 1 without losing the interaction. The other checks looked fine too: the outcome is binary with a 103/81 balance, the correlation matrix didn't show any other overlapping features, and the Box-Tidwell test passed for all four, so none needed changing. For the predictive side I kept the 5-fold cross-validation, looked at precision and recall instead of just accuracy, and compared logistic regression against a random forest before sticking with logistic regression.
